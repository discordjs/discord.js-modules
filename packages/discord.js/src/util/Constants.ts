...
function keyMirror(arr: any[]): object {
...
const AllowedImageSizes = Array.from({ length: 9 }, (_e: null, i: number) => 2 ** (i + 4));

/**
 * Options for Image URLs
 * @typedef {Object} ImageURLOptions
 * @property {string} [format] One of `webp`, `png`, `jpg`, `jpeg`, or `gif`. If no format is provided,
 * defaults to `webp`.
 * @property {boolean} [dynamic] If true, the format will dynamically change to `gif` for
 * animated avatars; the default is false.
 * @property {number} [size] One of `16`, `32`, `64`, `128`, `256`, `512`, `1024`, `2048`, or `4096`
 */
...
/**
 * The current status of the client. These are the available statuses:
 * * READY: 0
 * * CONNECTING: 1,
 * * RECONNECTING: 2
 * * IDLE: 3
...
/**
 * The current status of a voice connection. These are the available statuses:
 * * CONNECTED
 * * CONNECTING
 * * AUTHENTICATING
 * * RECONNECTING
 ...
 * * MESSAGE
 * * REACTION
 * <warn>Partials require you to put checks in place when handling data. Read the Partials topic
 * listed in the sidebar for more information.</warn>
 ...
 */
export const ActivityTypes = ['PLAYING', 'STREAMING', 'LISTENING', 'WATCHING', 'CUSTOM_STATUS'];
...
/**
 * The value set for a guild's default message notifications, e.g. `ALL`. These are the available types:
 * * ALL
 * * MENTIONS
